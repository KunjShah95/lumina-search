...
export class PDFExportManager {
  ...
  private getStylesheet(theme: 'light' | 'dark'): string {
    ...
    return new Promise((resolve, reject) => {
      try {
        const doc = new PDFDocument({
          ...
          margin: 50,
          info: {
            Title: 'Lumina Search Report',
            Author: options.author || 'Lumina Search',
            Creator: 'Lumina Search PDF Export',
          },
        })
        ...
        doc.fontSize(24).text('Lumina Search Report', { align: 'center' })
        doc.moveDown()
        doc.fontSize(12).text(`Generated: ${new Date().toLocaleDateString()}`, { align: 'center' })
        doc.moveDown(2)

        const citations = this.extractCitationsFromHTML(htmlContent)
        const content = this.extractContentFromHTML(htmlContent)

        if (content.title) {
          doc.fontSize(18).text(content.title)
          doc.moveDown()
        }

        if (content.query) {
          doc.fontSize(12).text(`Query: ${content.query}`)
          doc.moveDown()
        }

        if (content.response) {
          doc.fontSize(14).text('Response', { underline: true })
          doc.moveDown(0.5)
          doc.fontSize(11).text(content.response, {
            align: 'justify',
            lineGap: 4,
          })
          doc.moveDown()
        ...
        if (content.sources && content.sources.length > 0) {
          ...
          content.sources.forEach((source, index) => {
            doc.fontSize(11).text(`${index + 1}. ${source.title}`)
            doc.fontSize(9).fillColor('blue').text(source.url, { link: source.url })
            doc.fillColor('black')
            if (source.snippet) {
              doc.fontSize(9).text(source.snippet, { indent: 20 })
            }
            doc.moveDown(0.5)
          })

          doc.moveDown()
          doc.fontSize(14).text('Citations', { underline: true })
          doc.moveDown(0.5)
        ...
        doc.text('Generated by Lumina Search', { align: 'center' })
        doc.text('Please verify information from original sources.', { align: 'center' })
   ...
   private extractContentFromHTML(html: string): {
    ...
    const queryMatch = html.match(/<strong>Query:<\/strong>\s*([^<]+)/i)
    if (queryMatch) result.query = queryMatch[1].trim()

    const responseMatch = html.match(/<div class="response-content">([\s\S]*?)<\/div>/i)
    if (responseMatch) {
    ...
    const sourceMatches = html.matchAll(/<li class="source-item">[\s\S]*?<a[^>]*>([^<]+)<\/a>[\s\S]*?<p class="source-url">([^<]+)<\/p>[\s\S]*?<p class="source-snippet">([^<]+)<\/p>/gi)
   ...
   private extractCitationsFromHTML(html: string): string[] {
    const citations: string[] = []
    const citationMatches = html.matchAll(/<li class="citation">([^<]+)<\/li>/gi)
